...
async def async_setup(hass, config):
    ...
    energy_tariff = EnergyTariff(hass, conf[CONF_TARIFF_PERIOD]) 
    ...
    def bi_horario_semanal(self, time):
        ...
        if time.weekday() == 5: 
            ...
            if i_verao <= time.replace(tzinfo=None) < f_verao:
                ...
                if 0 <= time.hour < 9:
                    return STATE_VAZIO_NORMAL
                if time.hour == 9 and time.minutes < 30:
                    return STATE_VAZIO_NORMAL
                elif 13 <= time.hour < 18:      
                    return STATE_VAZIO_NORMAL
                if time.hour == 18 and time.minutes < 30:
                    return STATE_VAZIO_NORMAL
                elif 22 <= time.hour < 24:      
                    return STATE_VAZIO_NORMAL
        if time.weekday() == 6:
